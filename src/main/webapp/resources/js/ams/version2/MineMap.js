...
function flagField(id){
	...
	var row = Number(id.split('/')[0]);
	var col = Number(id.split('/')[1]);
	data += '<img src="/optigram/resources/imgs/ams/flag.png" onmousedown="holdCheck(id)" onmouseup="mouseCheck(id)" class ="flag" id =';
	data += id;
	data += '>';
	sweeper--;
	leftMine--;
		...
		for(var j=0; j<size; j++){
			...
			if(answerField[i][j] == 10){
				...
				if(answerField[i][j] == 9){
					ans += '<td><img src="/optigram/resources/imgs/ams/mine2.png"></td>';
